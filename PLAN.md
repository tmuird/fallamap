...
### P1 — major
- [ ] T1.1 Graceful degradation: surface a clear "community features offline" state when Supabase is unreachable (today: silent empty states; configured host is NXDOMAIN — see Blockers)
- [ ] T1.2 Real moderation pipeline: insert `status:'pending'` in both hooks; move admin fetches behind the gate; enforce admin server-side in RLS (AUDIT §4)
