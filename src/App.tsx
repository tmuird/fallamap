...
import PostRequestPage   from "@/components/marketplace/PostRequestPage";
      ...
      >
        ...
        <main className="flex-grow flex flex-col relative">
          <AnimatePresence mode="wait" initial={false}>
            <Routes location={location} key={location.pathname}>
              <Route path="/"            element={<PageWrapper><MarketplaceHome /></PageWrapper>} />
              <Route path="/browse"      element={<PageWrapper><BrowsePage      /></PageWrapper>} />
              <Route path="/request/new" element={<PageWrapper><PostRequestPage /></PageWrapper>} />
        ...
        <footer className="w-full flex flex-col items-center justify-center py-12 px-6 border-t-2 border-falla-ink/5 mt-auto bg-falla-paper transition-colors duration-300">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-7 h-7 bg-falla-fire rounded-lg border-2 border-falla-ink flex items-center justify-center shadow-solid-sm">
              <Scissors size={13} weight="bold" className="text-white" />
            </div>
            <span className="font-display text-2xl tracking-widest text-falla-fire">craftly</span>
          </div>
          <p className="text-[10px] font-bold tracking-[0.3em] text-falla-ink/25 uppercase">
            © 2026 · Bespoke craft marketplace
          </p>
        </footer>
      </ClerkProvider>
    </div>
